...
function detectIntent(msg) {
  msg = msg.toLowerCase();
  if (msg.includes("duplicate") || msg.includes("copy") || msg.includes("clone")) return "duplicate";
...
// Canonical → default display name
  ...
  if (idx !== -1) {
    const original = updatedJson[key][idx];
    const newName = getNextLayerName(key);

    // Duplicate with new id + new name, but same dimensions/position
    updatedJson[key].push({
      ...original,
      id: `${key}-${Date.now()}`, 
      dimensions: { ...original.dimensions, layerName: newName },
      position: updateObj.position || original.position
    });
    return; // ✅ skip normal create logic
  }
}
      if (intent === "update") {
        const idx = updatedJson[key].findIndex(l =>
...
if (intent === "create") {
  ...
  // For duplication, include layers mentioned in the message if they exist
...
Update Rules:
- Always preserve the original "layerName" and "id" for updates. Do not rename or suffix them.
- Only modify fields explicitly requested by the user (e.g., width, height, position, visibility).
- Never create a new layer unless the intent is "create" or "duplicate".

Duplicate Rules:
- Preserve the original layer’s full structure and duplicate only the referenced layer(s) from Relevant JSON.   
- If user request changes, apply only requested modifications.
- If the user specifies a new position → update the position.  
- If the user specifies other changes (e.g., rotation, size, visibility) → apply them.
- "text" must explain which layer(s) were duplicated and what modifications were applied.  
- "data" must contain the duplicated layer(s) only, with new IDs and new names.
